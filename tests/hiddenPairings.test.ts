...
import { getRow, getColumn, getBox, getOpen } from '../src/utils/cellPath'
import { applySolution, Solution } from '../src/utils/solutionObject'
import {
  solveHiddenPair,
  solveHiddenTriple,
  solveHiddenQuad,
} from '../src/strategies/hiddenPairings'
import {
  incorrectGrid2,
  hiddenPairGrid,
  hiddenTripleGrid,
...
describe('Solve Hidden Pairings', function () {
  describe('Solve Hidden Pairs', function () {
    it('correct single solution', function () {
      const solution = solveHiddenPair(
        hiddenPairGrid,
        getColumn(0)
      ) as Solution[]
  ...
  describe('Solve Hidden /Cascading Triples', function () {
    it('correct single solution', function () {
      const solution = solveHiddenTriple(
        hiddenTripleGrid,
        getRow(9)
      ) as Solution[]
      const SL1 = solution[0]
      ...
      const count3s = getOpen(getBox(23), updatedGrid).filter((x) =>
        (updatedGrid[x] as number[]).includes(3)
      )
  ...
  describe('Solve Hidden /Cascading Quads', function () {
    it('correct single solution', function () {
      const solution = solveHiddenQuad(hiddenQuadGrid, getBox(15)) as Solution[]
      const SL1 = solution[0]
      ...
      const count2s = getOpen(getRow(9), updatedGrid).filter((x) =>
        (updatedGrid[x] as number[]).includes(2)
      )
