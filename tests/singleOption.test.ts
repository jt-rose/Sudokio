...
import solveSingleOption from '../src/strategies/singleOption'
import { singleOptionGrid } from './gridSamplesForTesting'
...
describe('1. Solve Single Option', function () {
  it('valid solution for single parameter - row', function () {
    const solution = solveSingleOption(singleOptionGrid, 5) as Solution
    assert.sameOrderedMembers(singleOptionGrid[5] as number[], [6])
  ...
  it('valid solution for single parameter - column', function () {
    const solution2 = solveSingleOption(singleOptionGrid, 63) as Solution
    assert.sameOrderedMembers(singleOptionGrid[63] as number[], [8])
  ...
  it('valid solution for single parameter - box', function () {
    const solution3 = solveSingleOption(singleOptionGrid, 16) as Solution
    assert.sameOrderedMembers(singleOptionGrid[16] as number[], [5])
  ...
  it('valid solution for multiple parameters', function () {
    const solution4 = solveSingleOption(singleOptionGrid, 22) as Solution
    assert.sameOrderedMembers(singleOptionGrid[22] as number[], [2])
  ...
  it('valid solution for multiple parameters assisted by advanced strategy narrowing', function () {
    ...
    const solution5 = solveSingleOption(gridArray3, 80) as Solution
