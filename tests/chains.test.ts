...
import { solveXChain, solveXChainFullGrid } from '../src/strategies/chains'
import { xChainGrid, unansweredGrid } from './gridSamplesForTesting'
...
describe('Solve Force Chains', function () {
  it('correct force chain of length 2, applied to single cell', function () {
    ...
    const xChainAnswer = solveXChain(xChainGrid, 30) as Solution
    assert.equal(xChainAnswer.updates[0].index, 22)
    assert.sameOrderedMembers(xChainAnswer.updates[0].currentAnswer, [
      3,
      5,
      8,
      9,
    ...
    assert.sameOrderedMembers(xChainAnswer.updates[1].currentAnswer, [
      3,
      5,
      8,
      9,
  ...
  it('correct force chain of length 2, applied to full grid', function () {
    ...
    const cellsWithXChainFound = xChainFullGridAnswer.map(
      (solution) => solution.cellInit[0]
    )
    assert.sameOrderedMembers(expectedCellsWithXChain, cellsWithXChainFound)

    const expectedRoundsTaken = [3, 2, 1, 3, 3, 4, 3, 1, 2, 1, 1, 3]
    const actualRoundsTaken = xChainFullGridAnswer.map(
    ...
    const actualFirstUpdatesWithCurrentAnswers = xChainFullGridAnswer.map(
      (solution) => solution.updates[0].currentAnswer
    )
    assert.sameDeepOrderedMembers(
      expectedFirstUpdatesWithCurrentAnswers,
      actualFirstUpdatesWithCurrentAnswers
    )
    ...
    const actualFirstUpdatesWithUpdatedAnswers = xChainFullGridAnswer.map(
      (solution) => solution.updates[0].updatedAnswer
    )
    assert.sameDeepOrderedMembers(
      expectedFirstUpdatesWithUpdatedAnswers,
      actualFirstUpdatesWithUpdatedAnswers
    )
