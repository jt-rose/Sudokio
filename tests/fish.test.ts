...
describe('Solve Fish Strategies', function () {
  describe('Solve X-Wing', function () {
    it('correct solution', function () {
      const solution = solveXWing(XWingGrid1) as Solution[]
      ...
      const solution2 = solveXWing(XWingGrid2) as Solution[]
    ...
    it('correct rejection', function () {
      const reject = solveXWing(incorrectGrid)
      assert.equal(reject, false)
    })
  })
  describe('Solve Swordfish', function () {
    it('correct solution', function () {
      const solution = solveSwordfish(swordfishGrid1) as Solution[]
      const SL1 = solution[0]

      assert.equal(solution.length, 1)

      assert.equal(SL1.strategy, 'Swordfish')
      assert.sameOrderedMembers(SL1.cellInit, [
        9,
        18,
        27,
        13,
      ...
      assert.sameOrderedMembers(SL1.updates[4].currentAnswer, [
        1,
        4,
        5,
        6,
      ...
      const solution2 = solveSwordfish(swordfishGrid2) as Solution[]
    ...
    it('correct rejection', function () {
      const reject = solveSwordfish(incorrectGrid)
      assert.equal(reject, false)
    })
  })
  describe('Solve JellyFish', function () {
    it('correct solution', function () {
      const solution = solveJellyfish(jellyfishGrid1) as Solution[]
      const SL1 = solution[0]
      ...
      const solution2 = solveJellyfish(jellyfishGrid2) as Solution[]
      const SL2 = solution2[0]
      ...
      assert.sameOrderedMembers(SL2.cellInit, [
        18,
        21,
        26,
        45,
      ...
      assert.sameMembers(SL3.cellInit, [
        1,
        28,
        37,
        64,
    ...
    it('correct rejection', function () {
      const reject = solveJellyfish(incorrectGrid)
      assert.equal(reject, false)
    })
  })
})
