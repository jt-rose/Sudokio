...
import { getBox } from '../src/utils/cellPath'
import { Solution } from '../src/utils/solutionObject'
import solveBoxNarrow from '../src/strategies/boxNarrow'
import {
  boxNarrowGrid1,
  boxNarrowGrid2,
  unansweredGrid,
...
const solutionList1 = solveBoxNarrow(boxNarrowGrid1, getBox(40)) as Solution[]
const solutionList2 = solveBoxNarrow(boxNarrowGrid2, getBox(0)) as Solution[]
...
describe('3. Solve boxNarrow', function () {
  it('valid solutions list for single-row answers', function () {
    ...
    assert.sameOrderedMembers(solutionList1[1].cellInit as number[], [48, 49])
    ...
    assert.sameOrderedMembers(solutionList1[1].narrow[1].updatedAnswer, [
      5,
      6,
      7,
    ])
    ...
    assert.sameOrderedMembers(solutionList1[3].cellInit as number[], [48, 49])
    ...
    assert.sameOrderedMembers(solutionList2[0].cellInit as number[], [18, 19])
    ...
    assert.sameOrderedMembers(solutionList2[1].cellInit as number[], [1, 2])
  ...
  it('valid solutions list for single-column answers', function () {
    ...
    assert.sameOrderedMembers(solutionList1[0].cellInit as number[], [32, 41])
    assert.equal(solutionList1[0].narrow.length, 2)
    assert.equal(solutionList1[0].narrow[0].index, 59)
    assert.sameOrderedMembers(solutionList1[0].narrow[0].updatedAnswer, [
      3,
      7,
      8,
    ])
    assert.equal(solutionList1[0].narrow[1].index, 77)
    assert.sameOrderedMembers(solutionList1[0].narrow[1].updatedAnswer, [
      3,
      6,
      7,
      8,
    ...
    assert.sameOrderedMembers(solutionList1[2].cellInit as number[], [32, 41])
    assert.equal(solutionList1[2].narrow.length, 2)
    assert.equal(solutionList1[2].narrow[0].index, 59)
    assert.sameOrderedMembers(solutionList1[2].narrow[0].updatedAnswer, [
      1,
      7,
      8,
    ])
    assert.equal(solutionList1[2].narrow[1].index, 77)
    assert.sameOrderedMembers(solutionList1[2].narrow[1].updatedAnswer, [
      1,
      6,
      7,
      8,
    ...
    const solutionList2 = solveBoxNarrow(
      boxNarrowGrid2,
      getBox(76)
    ) as Solution[]
    ...
    // check for valid solution content (single-row) at start of array
    assert.equal(solutionList2[0].strategy, 'boxNarrow')
    assert.sameOrderedMembers(solutionList2[0].cellInit as number[], [59, 68])
    assert.equal(solutionList1[2].narrow.length, 2)
