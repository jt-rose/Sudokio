...
export const checkGridDataType = (gridString: string) =>
  typeof gridString === 'string'

// confirm gridString has 81 characters
export const checkGridLength = (gridString: string) => {
  return gridString.length === 81
}

// confirm only numbers, no other characters
export const checkOnlyNumbers = (gridString: string) => {
  return Array.from(gridString).every((char) => char.match(/[0-9]/))
}

// confirm at least 16 cells have been answered
export const checkMinimumAnswered = (gridString: string) => {
  return gridString.replace(/0/g, '').length >= 16
}

// confirm no contradictions EX: two 9's in same row
export const checkNoObviousErrors = (sudokuGrid: SudokuGrid) => {
...
export const isComplete = (sudokuGrid: SudokuGrid) => {
...
export const isCorrectSoFar = (sudokuGrid: SudokuGrid) => {
  const currentAnswerSets = CP.allSets.map((set) =>
    getSolved(set, sudokuGrid).map((x) => sudokuGrid[x])
  )

  return currentAnswerSets.every(
    (set) => set.length === [...new Set(set)].length && !set.includes(0)
  )
}

// checks grid is complete and correct
export const isCompleteAndCorrect = (sudokuGrid: SudokuGrid) =>
