...
export function hasSomeButNotAll(arr: any[], findValues: any[]) {
  return (
    arr.some((arrValue) => findValues.includes(arrValue)) &&
    !R.equals(arr, findValues)
  )
}

// determine strategy used, differentiating between naked and cascading pairings
function nakedPairStrat(
  pairLength: number,
  cellGroup: number[],
  ...
  return `Pair of length ${pairLength} Found`
}
...
const solveNakedPairings = (pairLength: number) => (
  sudokuGrid: SudokuGrid,
  gridParam: GridParam
) => {
  const openCells = getOpen(gridParam, sudokuGrid)
  const openCellSets = findGroups(pairLength)(openCells) as number[][]

  const answerSets = openCellSets
    .map((cellGroup) => ({
      cellGroup,
      answers: getUniqueOpenValues(cellGroup, sudokuGrid),
    }))
    // keep only sets with amount of answers that match the pairing length
    .filter((set) => set.answers.length === pairLength)
    .map((set) => ({
      ...set,
      removeFrom: getExternal(set.cellGroup, openCells).filter((x) =>
        hasSomeButNotAll(sudokuGrid[x] as number[], set.answers)
      ),
    }))
  ...
  const stratUsed = answerSets.map((x) =>
    nakedPairStrat(pairLength, x.cellGroup, sudokuGrid)
  )
  const updates = answerSets.map((set) =>
    set.removeFrom.map(
      (index) => new Update({ index, sudokuGrid, removal: set.answers })
    )
  )
  return answerSets.map(
    (obj, i) =>
      new Solution({
        strategy: stratUsed[i],
        cellInit: obj.cellGroup,
        updates: updates[i],
      })
  )
}
