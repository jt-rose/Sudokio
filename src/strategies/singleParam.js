import { getParam, getOpen, getValues } from '../utils/cellPath'

import { isOnly, formatUpdate, formatSolution } from '../utils/solutionObject'
...
export default function solveSingleParam(sudokuGrid, gridParam) {
  ...
  const indexFound = singleValuesFound.map((val) =>
    openGrid.find((x) => sudokuGrid[x].includes(val))
  )
