...
export default function solveBoxNarrow(
  ...
  const answerFoundAt = openAnswers.map((x) =>
    getOpenCellsWith(boxIndex, sudokuGrid, x)
  )
  const answerRows = answerFoundAt.map((x) => x.map((y) => getRowNumber(y)))
  const answerColumns = answerFoundAt.map((x) =>
    x.map((y) => getColumnNumber(y))
  )

  // narrow down to only answers found in single row or column of box
  const singleLineAnswers = openAnswers
    .map((answer, index) => ({
      answer,
    ...
    .map((obj) => ({
      ...
      externalIndex:
        obj.rows.length === 1
          ? getOpenExternalWith(
              boxIndex,
              getRow(obj.foundAt[0]),
              sudokuGrid,
              obj.answer
            )
          : getOpenExternalWith(
              boxIndex,
              getColumn(obj.foundAt[0]),
              sudokuGrid,
              obj.answer
            ),
    }))
    ...
    .map((obj) => ({
      ...obj,
      updates: obj.externalIndex.map(
        (index) => new Update({ index, sudokuGrid, removal: [obj.answer] })
      ),
    }))

  const solutionList = singleLineAnswers.map(
    (obj) =>
      new Solution({
        strategy: 'boxNarrow',
        cellInit: obj.foundAt,
        updates: obj.updates,
      })
  )
