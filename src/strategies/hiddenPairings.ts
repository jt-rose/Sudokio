...
function hiddenPairStrat(
  ...
  if (answerPairs.length === 3) {
    return cellGroup
      .map((x) => sudokuGrid[x] as number[])
      .every((x) => includesEach(x, answerPairs))
      ? 'hiddenTriple'
      : 'hiddenCascadingTriple'
  }
...
const solveHiddenPairings = (pairLength: number) => (
  sudokuGrid: SudokuGrid,
  gridParam: GridParam
) => {
  const answerSets = findGroups(pairLength)(
    getUniqueOpenValues(gridParam, sudokuGrid)
  ) as number[][]
  const openCells = getOpen(gridParam, sudokuGrid)

  const answerSetsFound = answerSets
    .map((answerSet) => ({
      answerSet,
      foundIn: openCells.filter((x) =>
        answerSet.some((answer) => (sudokuGrid[x] as number[]).includes(answer))
      ),
    }))
    // keep only cell-sets where amount of cells matches pairlength
    .filter((set) => set.foundIn.length === pairLength)
    .map((set) => ({
      ...set,
      // filter cells for removeFrom to keep only ones that include answers outside answerSet
      removeFrom: set.foundIn.filter((cell) =>
        (sudokuGrid[cell] as number[]).some((x) => !set.answerSet.includes(x))
      ),
    }))
  ...
  const updates = answerSetsFound.map((set) =>
    set.removeFrom.map(
      (index) =>
        new Update({ index, sudokuGrid, removal: isOnly(set.answerSet) })
    )
  )
  const solutionList = answerSetsFound.map(
    (set, i) =>
      new Solution({
        strategy: stratUsed[i],
        cellInit: set.foundIn,
        updates: updates[i],
      })
  )
  return solutionList
